<?php
    ...
    function tools_get($node_name,$query,$conn,$show_errors,$structure){
    ...
    function tools_after_read($query_form,$tools_res,$table_name){
        $res = array(null,array());
        //nyd
        //db_read_indexed($conn,"SELECT * FROM hive ",$show_errors);
        if(count($tools_res[1])==0){
            //formulate sql to be exectuted
            $sql = mixer_construct_selection_sql($table_name,$tools_res);
            //execute the sql
            $db_res = db_run(array("sql" => $sql,"conn" => $conn));
            $res[EI] = array_merge($res[EI],$db_res[EI]);
            //build the data into honey structure
            $honey = mixer_construct_honey($db_res[RI]["data"]);
